...
import DEFAULT_OPERATIONS from './allCategoriesPage.gql';


export const useAllCategoriesPage = (props = {}) => {
    const operations = mergeOperations(DEFAULT_OPERATIONS, props.operations);
    const { getAllCategoriesQuery, getStoreConfigQuery } = operations;
    ...
    const megaMenuData = useMemo(() => {
        return data ? processData(data.categories.items[0]) : {};
    }, [data, processData]);
